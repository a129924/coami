    ...
    with TestClient(create_app()) as client:
        ...
        with client.websocket_connect(f"/v1/devices/{DEVICE_ID}/session") as socket:
            ...
            assert socket.receive_json() == {
                "type": "robot.command", "command_id": command_id, "action": "greet"
            }
            ...
            assert client.get(f"/v1/commands/{command_id}").json()["status"] == "completed"
...
def test_stop_cancels_greet_and_disconnect_fails_pending() -> None:
    with TestClient(create_app()) as client:
        with client.websocket_connect(f"/v1/devices/{DEVICE_ID}/session") as socket:
            socket.send_json({"type": "robot.hello", "device_id": DEVICE_ID, "version": 1})
            socket.receive_json()
            greet = client.post(f"/v1/devices/{DEVICE_ID}/actions/greet").json()["command_id"]
            socket.receive_json()
            assert client.post(f"/v1/devices/{DEVICE_ID}/actions/greet").status_code == 409
            stop = client.post(f"/v1/devices/{DEVICE_ID}/actions/stop").json()["command_id"]
            assert socket.receive_json()["command_id"] == stop
            socket.send_json({"type": "robot.command_result", "command_id": greet, "status": "cancelled"})
            socket.send_json({"type": "robot.command_result", "command_id": stop, "status": "completed"})
